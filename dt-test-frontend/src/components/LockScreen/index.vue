  ...
  <section class="LockScreen" v-if="lockScreen" :class="afterUnlock">
    ...
    <div class="content">
      ...
      <div class="deviceStatus">
        ...
        <a-tooltip>
          <template #title>{{
            isClickedUnlockBtn ? "返回" : "点击解锁"
          }}</template>
          <LockOutlined
            class="unlock"
            @click="onClickLockBtn"
            v-if="isClickedUnlockBtn"
          />
...
const userinfo = { avatar: "/avatar.jpg", username: "" };
const initBg = "/back.jpg";
...
export default defineComponent({
  ...
  setup() {
    ...
    const lockScreen = computed(() => {
      return state.app.lockScreen;
    });

    // const lockScreen = state.app.lockScreen
    commit(SET_LOCK_SCREEN, storage.get(SET_LOCK_SCREEN));
    // const lockScreen = state.app.lockScreen
    ...
    const onChangeBg = (file) => {
      ...
      reader.onload = function () {
        indexdb.clearTable("indexDBname","tablename", (res) => {}); //清空库表，不是清库，是清表数据
        indexdb.insertData(
          "indexDBname",
          "tablename",
          "indexkey",
    ...
    const onUnlockScreen = async () => {
      ...
      storage.set(SET_LOCK_SCREEN, false);
    };
