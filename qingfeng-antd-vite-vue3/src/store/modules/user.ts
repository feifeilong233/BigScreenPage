            ...
            return new Promise((resolve, reject) => {
                getInfo().then(res => {
                    ...
                    if (response.role && response.role.permissions.length > 0) {
                        ...
                        const permissionButtonList = response.authList.map(authData => {
                            return authData.parent_permission + ':' + authData.permission
                        })

                        commit('SET_AUTHORGANIZE', response.orgPd)
                        commit('SET_AUTHBUTTON', permissionButtonList)
                        role.permissionButtonList = permissionButtonList;
                        role.authOrganize = response.orgPd;
